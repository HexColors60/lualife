-- Expansionist AI for faction ai_16
-- Rapid expansion
...
if not memory then
    memory = {
        ...
        tick_count = 0,
        last_log_tick = 0,
        priority_resources = {"power", "iron"},
        aggression = 0.5,
    }
end

function on_init()
    log.info("AI 16 Expansionist initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 16 Expansionist: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Expansionist: move outward
    if memory.tick_count % 60 == 0 then units.move_to(unit_id, math.min(255, unit.x + 5), math.min(255, unit.y + 5)) return end
...
function on_unit_spawned(unit_id)
    log.info("AI 16: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 16: Unit died: " .. tostring(unit_id))
end
