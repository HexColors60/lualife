...
function on_init()
    log.info("AI 27 Passive initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 27 Passive: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Passive behavior
    -- Passive: avoid all conflict
    -- Just gather peacefully
...
function on_unit_spawned(unit_id)
    log.info("AI 27: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 27: Unit died: " .. tostring(unit_id))
end
