...
if not memory then
    memory = {
        ...
        tick_count = 0,
        last_log_tick = 0,
        priority_resources = {"power", "carbon"},
        aggression = 0.95,
    }
end

function on_init()
    log.info("AI 08 Rusher initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 08 Rusher: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Rusher behavior
    -- Rusher: attack quickly
    local enemy = find_nearest_enemy(unit) if enemy then units.attack(unit_id, enemy.id) return end
...
function on_unit_spawned(unit_id)
    log.info("AI 08: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 08: Unit died: " .. tostring(unit_id))
end
