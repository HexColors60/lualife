...
if not memory then
    memory = {
        ...
        tick_count = 0,
        last_log_tick = 0,
        priority_resources = {"power"},
        aggression = 0.6,
    }
end

function on_init()
    log.info("AI 10 Swarm initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 10 Swarm: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Swarm behavior
    -- Swarm: gather then attack
    if memory.tick_count % 100 < 50 then local m = find_nearest_mine(unit, "power") if m then units.mine(unit_id, m.id) return end else local e = find_nearest_enemy(unit) if e then units.attack(unit_id, e.id) return end end
...
function on_unit_spawned(unit_id)
    log.info("AI 10: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 10: Unit died: " .. tostring(unit_id))
end
