...
if not memory then
    memory = {
        ...
        tick_count = 0,
        last_log_tick = 0,
        priority_resources = {"crystal", "power"},
        aggression = 0.0,
    }
end

function on_init()
    log.info("AI 14 Diplomat initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 14 Diplomat: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Diplomat behavior
    -- Diplomat: avoid conflict, gather
    -- Peaceful resource gathering
...
function on_unit_spawned(unit_id)
    log.info("AI 14: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 14: Unit died: " .. tostring(unit_id))
end
