...
if not memory then
    memory = {
        ...
        tick_count = 0,
        last_log_tick = 0,
        priority_resources = {"stone", "carbon"},
        aggression = 0.15,
    }
end

function on_init()
    log.info("AI 03 Builder initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 03 Builder: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Builder behavior
    -- Builder: look for construction sites
    local site = find_construction_site(unit)
    if site and has_part(unit, "build") then units.build(unit_id, site.id) return end
...
function on_unit_spawned(unit_id)
    log.info("AI 03: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 03: Unit died: " .. tostring(unit_id))
end
