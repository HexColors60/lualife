...
if not memory then
    memory = {
        ...
        tick_count = 0,
        last_log_tick = 0,
        priority_resources = {"silicon", "crystal"},
        aggression = 0.2,
    }
end

function on_init()
    log.info("AI 11 Tech initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 11 Tech: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Tech behavior
    -- Tech: gather tech resources
    for _, r in ipairs({"silicon", "crystal"}) do local m = find_nearest_mine(unit, r) if m then units.mine(unit_id, m.id) return end end
...
function on_unit_spawned(unit_id)
    log.info("AI 11: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 11: Unit died: " .. tostring(unit_id))
end
