-- Determined AI for faction ai_24
-- Never retreats
...
if not memory then
    memory = {
        ...
        tick_count = 0,
        last_log_tick = 0,
        priority_resources = {"iron", "sulfur"},
        aggression = 1.0,
    }
end

function on_init()
    log.info("AI 24 Determined initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 24 Determined: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Determined behavior
    -- Determined: always fight
    if has_part(unit, "fight") then local e = find_nearest_enemy(unit) if e then units.attack(unit_id, e.id) return end end
...
function on_unit_spawned(unit_id)
    log.info("AI 24: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 24: Unit died: " .. tostring(unit_id))
end
