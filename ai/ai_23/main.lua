-- Opportunist AI for faction ai_23
-- Attacks weakened foes
...
if not memory then
    memory = {
        ...
        tick_count = 0,
        last_log_tick = 0,
        priority_resources = {"sulfur"},
        aggression = 0.5,
    }
end

function on_init()
    log.info("AI 23 Opportunist initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 23 Opportunist: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Opportunist: attack weak enemies
    local enemies = world.get_enemies() if enemies then for _, e in ipairs(enemies) do if e.hp and e.hp < 50 then units.attack(unit_id, e.id) return end end end
...
function on_unit_spawned(unit_id)
    log.info("AI 23: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 23: Unit died: " .. tostring(unit_id))
end
