-- Calculator AI for faction ai_20
-- Optimal decisions
...
if not memory then
    memory = {
        ...
        tick_count = 0,
        last_log_tick = 0,
        priority_resources = {"silicon", "power"},
        aggression = 0.4,
    }
end

function on_init()
    log.info("AI 20 Calculator initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 20 Calculator: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Calculator behavior
    -- Calculator: optimal choices
    if unit.power and unit.power < 30 then local m = find_nearest_mine(unit, "power") if m then units.mine(unit_id, m.id) return end end if math.random() < 0.3 and has_part(unit, "fight") then local e = find_nearest_enemy(unit) if e then units.attack(unit_id, e.id) return end end
...
function on_unit_spawned(unit_id)
    log.info("AI 20: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 20: Unit died: " .. tostring(unit_id))
end
