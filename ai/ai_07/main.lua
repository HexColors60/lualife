...
function on_init()
    log.info("AI 07 Tank initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 07 Tank: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Tank behavior
    -- Tank: seek combat
    if has_part(unit, "fight") then local enemy = find_nearest_enemy(unit) if enemy then units.attack(unit_id, enemy.id) return end end
...
function on_unit_spawned(unit_id)
    log.info("AI 07: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 07: Unit died: " .. tostring(unit_id))
end
