...
if not memory then
    memory = {
        ...
        tick_count = 0,
        last_log_tick = 0,
        priority_resources = {"iron", "copper"},
        aggression = 0.1,
    }
end

function on_init()
    log.info("AI 18 Hoarder initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 18 Hoarder: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Hoarder behavior
    -- Hoarder: stockpile all resources
    local resources = {"iron", "copper", "silicon", "crystal"} for _, r in ipairs(resources) do local m = find_nearest_mine(unit, r) if m then units.mine(unit_id, m.id) return end end
...
function on_unit_spawned(unit_id)
    log.info("AI 18: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 18: Unit died: " .. tostring(unit_id))
end
