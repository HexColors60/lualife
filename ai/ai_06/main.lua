-- Scavenger AI for faction ai_06
...
function on_init()
    log.info("AI 06 Scavenger initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 06 Scavenger: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Scavenger behavior
    -- Scavenger: prioritize power
    local mine = find_nearest_mine(unit, "power") if mine then units.mine(unit_id, mine.id) return end
...
function on_unit_spawned(unit_id)
    log.info("AI 06: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 06: Unit died: " .. tostring(unit_id))
end
