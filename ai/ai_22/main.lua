-- Territorial AI for faction ai_22
-- Defends territory
...
if not memory then
    memory = {
        ...
        tick_count = 0,
        last_log_tick = 0,
        priority_resources = {"iron", "stone"},
        aggression = 0.6,
    }
end

function on_init()
    log.info("AI 22 Territorial initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 22 Territorial: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    local enemies = world.get_enemies() if enemies then for _, e in ipairs(enemies) do if distance(unit.x, unit.y, e.x, e.y) < 15 then units.attack(unit_id, e.id) return end end end
...
function on_unit_spawned(unit_id)
    log.info("AI 22: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 22: Unit died: " .. tostring(unit_id))
end
