...
function on_init()
    log.info("AI 09 Turtle initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 09 Turtle: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Turtle behavior
    -- Turtle: gather defensively
    local mine = find_nearest_mine(unit, "stone") if mine then units.mine(unit_id, mine.id) return end
...
function on_unit_spawned(unit_id)
    log.info("AI 09: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 09: Unit died: " .. tostring(unit_id))
end
