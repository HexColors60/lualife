...
if not memory then
    memory = {
        ...
        tick_count = 0,
        last_log_tick = 0,
        priority_resources = {"sulfur", "crystal"},
        aggression = 0.75,
    }
end

function on_init()
    log.info("AI 19 Gambler initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 19 Gambler: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Gambler behavior
    -- Gambler: risky combat
    if math.random() < 0.4 then local e = find_nearest_enemy(unit) if e then units.attack(unit_id, e.id) return end end
...
function on_unit_spawned(unit_id)
    log.info("AI 19: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 19: Unit died: " .. tostring(unit_id))
end
