-- Minimalist AI for faction ai_17
-- Efficient few units
...
function on_init()
    log.info("AI 17 Minimalist initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 17 Minimalist: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Minimalist behavior
    -- Minimalist: efficient actions only
    if unit.power and unit.power < 20 then local m = find_nearest_mine(unit, "power") if m then units.mine(unit_id, m.id) return end end
...
function on_unit_spawned(unit_id)
    log.info("AI 17: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 17: Unit died: " .. tostring(unit_id))
end
