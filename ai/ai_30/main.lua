-- Symbiotic AI for faction ai_30
-- Works with allies
...
if not memory then
    memory = {
        ...
        tick_count = 0,
        last_log_tick = 0,
        priority_resources = {"water", "power"},
        aggression = 0.2,
    }
end

function on_init()
    log.info("AI 30 Symbiotic initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 30 Symbiotic: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Symbiotic behavior
    -- Symbiotic: cooperative
    local m = find_nearest_mine(unit, "power") if m then units.mine(unit_id, m.id) return end
...
function on_unit_spawned(unit_id)
    log.info("AI 30: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 30: Unit died: " .. tostring(unit_id))
end
