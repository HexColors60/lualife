...
function on_init()
    log.info("=== EVOLUTION SIMULATION STARTED ===")
    log.info(string.format("World: %dx%d, Initial creatures: %d", 
        WORLD_WIDTH, WORLD_HEIGHT, INITIAL_CREATURES))
    ...
    log.info("Generation 1 begun with " .. #evolution.creatures .. " creatures")
end
...
function on_tick()
    ...
    if evolution.tick % 50 == 0 then
        calculate_stats()
    end
    
    if evolution.tick % TICKS_PER_GENERATION == 0 then
        calculate_stats()
        log.info(string.format("[Gen %d] Pop: %d | Avg Speed: %.2f | Avg Size: %.2f | Avg Sense: %.2f",
    ...
    if #evolution.creatures == 0 then
        log.warn("EXTINCTION! All creatures have died.")
        for i = 1, 5 do
            table.insert(evolution.creatures, create_creature())
        end
        evolution.generation = evolution.generation + 1
        log.info("Restarted with 5 new creatures. Generation " .. evolution.generation)
    end
end
...
function get_status()
    ...
    for _, c in ipairs(evolution.creatures) do
        if c.genes.diet == DIET_HERBIVORE then herbivores = herbivores + 1
        elseif c.genes.diet == DIET_CARNIVORE then carnivores = carnivores + 1
        else omnivores = omnivores + 1 end
    end
    
    return {
        generation = evolution.generation,
        tick = evolution.tick,
        population = #evolution.creatures,
        food_count = #evolution.food,
        diet_breakdown = {
...
function get_top_creatures(n)
    local sorted = {}
    for _, c in ipairs(evolution.creatures) do
        table.insert(sorted, c)
    end
    table.sort(sorted, function(a, b) return a.energy > b.energy end)
    
    local top = {}
    for i = 1, math.min(n or 5, #sorted) do
        table.insert(top, {
            id = sorted[i].id,
            energy = sorted[i].energy,
            age = sorted[i].age,
            genes = sorted[i].genes,
            diet_name = get_diet_name(sorted[i].genes.diet),
        })
    end
    return top
end