-- Defensive AI for faction ai_04
...
if not memory then
    memory = {
        ...
        tick_count = 0,
        last_log_tick = 0,
        priority_resources = {"stone", "iron"},
        aggression = 0.1,
    }
end

function on_init()
    log.info("AI 04 Defensive initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 04 Defensive: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Defensive behavior
    -- Defensive: stay near base
    if math.random() < 0.3 then local m = find_nearest_mine(unit, "stone") if m then units.mine(unit_id, m.id) return end end
...
function on_unit_spawned(unit_id)
    log.info("AI 04: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 04: Unit died: " .. tostring(unit_id))
end
