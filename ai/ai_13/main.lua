-- Raider AI for faction ai_13
-- Attacks supply lines
...
function on_init()
    log.info("AI 13 Raider initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 13 Raider: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Raider behavior
    -- Raider: attack miners
    local enemies = world.get_enemies() if enemies then for _, e in ipairs(enemies) do if e.action == "mine" then units.attack(unit_id, e.id) return end end end
...
function on_unit_spawned(unit_id)
    log.info("AI 13: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 13: Unit died: " .. tostring(unit_id))
end
