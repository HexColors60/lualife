-- Parasitic AI for faction ai_31
-- Exploits others
...
if not memory then
    memory = {
        ...
        tick_count = 0,
        last_log_tick = 0,
        priority_resources = {"biomass"},
        aggression = 0.7,
    }
end

function on_init()
    log.info("AI 31 Parasitic initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 31 Parasitic: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Parasitic behavior
    -- Parasitic: follow others
    local enemies = world.get_enemies() if enemies and #enemies > 0 then local t = enemies[math.random(1, #enemies)] units.move_to(unit_id, t.x, t.y) return end
...
function on_unit_spawned(unit_id)
    log.info("AI 31: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 31: Unit died: " .. tostring(unit_id))
end
