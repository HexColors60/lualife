-- Isolationist AI for faction ai_15
-- Stays in territory
...
if not memory then
    memory = {
        ...
        tick_count = 0,
        last_log_tick = 0,
        priority_resources = {"stone"},
        aggression = 0.0,
    }
end

function on_init()
    log.info("AI 15 Isolationist initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 15 Isolationist: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Isolationist: minimal movement
    if unit.power and unit.power < 50 then local m = find_nearest_mine(unit, "power") if m then units.mine(unit_id, m.id) return end end
...
function on_unit_spawned(unit_id)
    log.info("AI 15: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 15: Unit died: " .. tostring(unit_id))
end
