...
function on_init()
    log.info("AI 25 Cautious initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 25 Cautious: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Cautious behavior
    -- Cautious: retreat when damaged
    if unit.hp and unit.hp < 30 then local m = find_nearest_mine(unit, "power") if m then units.mine(unit_id, m.id) return end end
...
function on_unit_spawned(unit_id)
    log.info("AI 25: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 25: Unit died: " .. tostring(unit_id))
end
