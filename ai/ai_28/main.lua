...
if not memory then
    memory = {
        ...
        tick_count = 0,
        last_log_tick = 0,
        priority_resources = {"crystal"},
        aggression = 0.5,
    }
end

function on_init()
    log.info("AI 28 Random initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 28 Random: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    local action = math.random(1, 3) if action == 1 then local m = find_nearest_mine(unit, memory.priority_resources[math.random(1, #memory.priority_resources)]) if m then units.mine(unit_id, m.id) return end elseif action == 2 then explore(unit_id, unit) return end
...
function on_unit_spawned(unit_id)
    log.info("AI 28: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 28: Unit died: " .. tostring(unit_id))
end
