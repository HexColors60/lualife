-- Explorer AI for faction ai_05
-- Roams to find resources
...
if not memory then
    memory = {
        ...
        tick_count = 0,
        last_log_tick = 0,
        priority_resources = {"crystal", "silicon"},
        aggression = 0.3,
    }
end

function on_init()
    log.info("AI 05 Explorer initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 05 Explorer: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Explorer behavior
    -- Explorer: move to new areas
    if memory.tick_count % 50 == 0 then explore(unit_id, unit) return end
...
function on_unit_spawned(unit_id)
    log.info("AI 05: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 05: Unit died: " .. tostring(unit_id))
end
