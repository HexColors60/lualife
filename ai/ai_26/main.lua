-- Warmonger AI for faction ai_26
-- Constant combat
...
if not memory then
    memory = {
        ...
        tick_count = 0,
        last_log_tick = 0,
        priority_resources = {"sulfur", "iron"},
        aggression = 0.95,
    }
end

function on_init()
    log.info("AI 26 Warmonger initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 26 Warmonger: Tick %%d, Units: %%d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Warmonger behavior
    -- Warmonger: constant combat
    local e = find_nearest_enemy(unit) if e then units.attack(unit_id, e.id) return end
...
function on_unit_spawned(unit_id)
    log.info("AI 26: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 26: Unit died: " .. tostring(unit_id))
end
