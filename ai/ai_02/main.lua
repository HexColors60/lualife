-- Economic AI for faction ai_02
-- Focuses on resource gathering and stockpiling
...
if not memory then
    memory = {
        ...
        tick_count = 0,
        last_log_tick = 0,
        priority_resources = {"iron", "copper", "silicon"},
        aggression = 0.1,
    }
end

function on_init()
    log.info("AI 02 Economic initialized!")
...
function on_tick()
    ...
    if memory.tick_count - memory.last_log_tick >= 500 then
        ...
        log.info(string.format("AI 02 Economic: Tick %d, Units: %d", memory.tick_count, #owned_units))
    end
end
...
function process_unit(unit_id, unit)
    ...
    -- Economic: gather priority resources
...
function on_unit_spawned(unit_id)
    log.info("AI 02: Unit spawned: " .. tostring(unit_id))
end

function on_unit_died(unit_id)
    log.warn("AI 02: Unit died: " .. tostring(unit_id))
end